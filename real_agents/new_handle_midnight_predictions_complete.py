    async def handle_midnight_predictions(self, request):
        ...
        try:
            ...
            live_league = request.query.get('league', 'ALL').upper()
            ...
            # 🔥💀🔥 FIX: Handle ALL league the same as Season Analysis! 💀🔥💀
            league_filter = live_league if live_league != 'ALL' else None
            
            # Check if automation is active for this league
            has_automation_data = midnight_reader.is_automation_active(league_filter)
            ...
            season_data = midnight_reader.get_season_predictions(league_filter)
            accuracy_stats = midnight_reader.get_accuracy_stats(league_filter)
